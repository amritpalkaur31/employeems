    ...
    const navigate = useNavigate()
        ...
        useEffect(() => {
            const fetchLeave = async () => {
              try {
                const response = await axios.get(`https://employeems-backend-x0a7.onrender.com/api/leave/detail/${id}`, 
                    {
                  headers: {
...
const changeStatus = async (id, status) => {
    try {
        const response = await axios.put(`https://employeems-backend-x0a7.onrender.com/api/leave/${id}`, {status},
            {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
...
<div className='flex-1 mt-16 p-4 max-w-3xl mx-auto bg-white rounded-md shadow-md'>        <h2 className='text-2xl font-bold mb-8 text-center'>
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
        <div> 
        <img src={`https://employeems-backend-x0a7.onrender.com/${leave.employeeId.userId.profileImage}`} 
